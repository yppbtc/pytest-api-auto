...
class RequestBase:
    ...
    def __init__(self):
        self.run = SendRequest()      # 发送HTTP请求
        self.conf = OperationConfig() # 读取配置
        self.read = ReadYamlData()    # 读写YAML文件
        self.asserts = Assertions()   # 断言验证

# 为什么要进行变量替换？
# 因为 token 的值是动态的，不能直接写值
# 变量替换是为了实现接口间的数据关联。比如登录接口会提取 token 保存到 extract.yaml，后续接口需要使用这个 token 时，
# 通过 ${get_extract_data(token)} 变量引用，框架会自动从 extract.yaml 读取最新的值进行替换，这样每次测试都能获取到正确的 token。"
# " replace_load 是一个工具方法，专门处理包含 ${...} 格式变量的数据，自动将变量替换为实际值。"
...
# 变量替换和数据提取是两个相反的操作。变量替换发生在发送请求之前，从 extract.yaml 读取数
# 据临时替换 ${...} 变量，只影响本次请求，不修改文件本身。数据提取发生在发送请求之后，
# 从响应中提取数据保存到 extract.yaml 文件，供后续接口使用。简单说：replace_load 是'读出来
# 替换'，extract_data 是'提取出来保存'。
